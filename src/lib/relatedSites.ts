...
/** 姉妹サイトにある「開業したらやること」の入口(母屋)。
 *
 * 2026-09-08 追加。S事業の2サイト(印鑑・請求書)は相互にリンクが通っているが、
 * 工程を1本に束ねた母屋を指す線だけが無かった。ここがその1本である。
 *
 * 印影を作りに来た人は、たいてい「これから初めて書類を出す人」である。
 * 次に必要になるのは見積書と請求書で、その順番はこの先にまとまっている。
 *
 * URL は追加時に本番で HTTP 200 を実測している(存在しないURLは書かない)。 */
export const STARTER_TOOLS_HUB = {
  label: "開業したらやること — 書類の順番と無料ツール",
  href: "https://invoice-tool-kohl.vercel.app/kaigyo",
} as const;
